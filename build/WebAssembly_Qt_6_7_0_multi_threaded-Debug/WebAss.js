...
var demangle = func => {
 ...
 return withStackSave(() => {
  try {
   ...
   if (GROWABLE_HEAP_I32()[((status) >>> 2) >>> 0] === 0 && ret) {
...
var PThread = {
 ...
 allocateUnusedWorker() {
  var worker;
  var pthreadMainJs = locateFile("WebAss.worker.js");
...
function ___syscall_chmod(path, mode) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(5, 1, path, mode);
 path >>>= 0;
 try {
...
function ___syscall_fallocate(fd, mode, offset, len) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(8, 1, fd, mode, offset, len);
...
function ___syscall_fchown32(fd, owner, group) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(10, 1, fd, owner, group);
 try {
...
function _fd_read(fd, iov, iovcnt, pnum) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(49, 1, fd, iov, iovcnt, pnum);
 iov >>>= 0;
 iovcnt >>>= 0;
 pnum >>>= 0;
 try {
...
function _fd_seek(fd, offset, whence, newOffset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(50, 1, fd, offset, whence, newOffset);
 offset = bigintToI53Checked(offset);
 newOffset >>>= 0;
 try {
  if (isNaN(offset)) return 61;
...
function _fd_sync(fd) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(51, 1, fd);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function _fd_write(fd, iov, iovcnt, pnum) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(52, 1, fd, iov, iovcnt, pnum);
 iov >>>= 0;
 iovcnt >>>= 0;
 pnum >>>= 0;
 try {
...
function _getaddrinfo(node, service, hint, out) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(53, 1, node, service, hint, out);
 node >>>= 0;
 service >>>= 0;
 hint >>>= 0;
 out >>>= 0;
...
var ___start_em_js = Module["___start_em_js"] = 10741008;

var ___stop_em_js = Module["___stop_em_js"] = 10742098;
